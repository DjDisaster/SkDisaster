...
public abstract class AsyncStoreEffect extends Effect {
    ...
    protected abstract Object getValue(Event event, String key);
    ...
    protected boolean initVariable(Expression<?> expr) {
        if (!(expr instanceof Variable)) {
            Skript.error("You must store the result in a variable.");
            return false;
        }

        variable = (Variable<?>) expr;
        if (variable.isList()) {
            Skript.error("You must store the result in a normal variable.");
            return false;
        }
