...
public class EffForwardToPlayer extends Effect {

    static {
        Skript.registerEffect(
                EffForwardToPlayer.class,
                "forward [proxy] payload %string% to [player] %string% on [channel] %string%"
        );
    }
    ...
    protected void execute(Event event) {
        ...
        String playerValue = player.getSingle(event);
        String channelValue = channel.getSingle(event);

        if (payloadValue == null || playerValue == null || channelValue == null) {
            return;
        }

        VelocityUtils.forwardToPlayer(playerValue, channelValue, payloadValue.getBytes(StandardCharsets.UTF_8));
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "forward proxy payload to player";
    }
}